...
export function InvoiceTable() {
  ...
  const bulkSendMutation = useMutation({
    ...
    onSuccess: (res) => {
      alert(`${res.data.data.updated}건 발송 완료`)
      setSelected(new Set())
      qc.invalidateQueries({ queryKey: ['invoices'] })
      qc.invalidateQueries({ queryKey: ['invoices-status-counts'] })
    },
  ...
  const draftSelected = visibleSelected.filter(i => i.status === 'draft')
      ...
      {selected.size > 0 && (
        <div className="flex flex-wrap items-center gap-3 rounded-2xl border border-blue-200 bg-blue-50/50 px-5 py-3 shadow-sm">
          ...
          <button
            ...
            title={draftSelected.length === 0 ? '발송 대상(draft)이 없습니다' : ''}
          >
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-50">
              {isLoading
                ...
                : invoices.length === 0 ? (
                    <tr><td colSpan={11} className="px-5 py-10 text-center text-sm text-gray-400">해당 탭에 청구서가 없습니다.</td></tr>
                  ) :
                  invoices.map(inv => {
                    ...
                    return (
                      <tr key={inv.id} className={clsx('hover:bg-gray-50 transition-colors', isChecked && 'bg-blue-50/40')}>
                        ...
                        <td className={`px-4 py-4 text-xs ${soonDue ? 'font-semibold text-rose-600' : 'text-gray-500'}`}>
                          {inv.due_date}{soonDue && <span className="ml-1">⚠️</span>}
                        </td>
                        <td className="px-4 py-4 text-xs text-emerald-700">
                          {inv.paid_at ? inv.paid_at.slice(0, 10) : <span className="text-gray-300">-</span>}
                        </td>
