...
export interface SettlementExportFilters {
  ...
  vendor_id?: string
  search?: string
  /** 사전입금 탭은 거래내역 기준이라 정산 엑셀 대상이 아님 */
  isPrepaidView: boolean
}

export function SettlementTable({ onFiltersChange }: { onFiltersChange?: (f: SettlementExportFilters) => void }) {
  ...
  // 목록 조회에 쓰는 필터를 그대로 엑셀 다운로드에도 넘긴다 (page/limit만 제외)
  const listFilters = {
  ...
  useEffect(() => {
    onFiltersChange?.({ ...listFilters, isPrepaidView })
    // listFilters는 매 렌더 새 객체라 원본 상태값으로 의존성을 건다
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [statusFilter, deptTab, search, isPrepaidView, onFiltersChange])

  const { data, isLoading } = useQuery({
    queryKey: ['settlements', page, statusFilter, deptTab, search],
    queryFn: () => settlementsApi.list({ page, limit: 20, ...listFilters }),
