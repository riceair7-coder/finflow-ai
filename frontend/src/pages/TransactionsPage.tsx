...
const IMPORT_PANEL_KEY = 'finflow.import_panel_collapsed'
...
/** 잘려 보이는 승인번호·설명 원문과 분류 근거를 확인하는 상세 패널 */
function TransactionDrawer({ tx, vendor, dept, onClose }: {
  tx: Transaction
  vendor?: { name: string; business_registration_no?: string; email?: string | null } | null
  dept?: { name: string; code?: string } | null
  onClose: () => void
}) {
  const conf = tx.ai_classification_confidence
  const basis = vendor?.business_registration_no
    ? `공급자 사업자등록번호(${vendor.business_registration_no}) 매핑`
    : tx.account_code
      ? `계정코드(${tx.account_code}) 규칙`
      : '수동 지정'

  const Row = ({ label, children }: { label: string; children: React.ReactNode }) => (
    <div className="grid grid-cols-[110px_1fr] gap-3 border-b border-gray-100 py-2.5 text-sm">
      ...
      >
        <div className="flex items-start justify-between border-b px-6 py-4">
          <div className="min-w-0">
            <h2 className="text-base font-semibold text-gray-900">거래 상세</h2>
            <p className="mt-0.5 truncate text-xs text-gray-400">{tx.transaction_date} · {SOURCE_LABELS[tx.source] ?? tx.source}</p>
          </div>
          <button onClick={onClose} aria-label="상세 닫기" className="rounded-lg p-1.5 text-gray-400 hover:bg-gray-100">✕</button>
        </div>

        <div className="flex-1 overflow-y-auto px-6 py-4">
          <p className="text-2xl font-bold text-gray-900">{fmt(tx.amount)}</p>
          <p className="mt-1 text-xs text-gray-400">
            공급가액 {tx.supply_amount != null ? fmt(Number(tx.supply_amount)) : '-'} · 세액 {tx.tax_amount != null ? fmt(Number(tx.tax_amount)) : '-'}
          </p>

          <dl className="mt-5">
            <Row label="설명">{tx.description || '-'}</Row>
            <Row label="승인번호"><span className="font-mono text-xs">{tx.external_id || '-'}</span></Row>
            <Row label="공급자">
              {vendor ? (
                <>
                  <span className="font-medium">{vendor.name}</span>
                  {vendor.business_registration_no && <span className="ml-1.5 font-mono text-xs text-gray-400">{vendor.business_registration_no}</span>}
                  {vendor.email && <p className="text-xs text-gray-400">{vendor.email}</p>}
                </>
              ) : '-'}
            </Row>
            <Row label="부서">{dept?.name ?? <span className="text-amber-600">미배정</span>}</Row>
            <Row label="계정코드">{tx.account_code || '-'}</Row>
            <Row label="상태">
            ...
            <Row label="정산상태">
              {tx.is_settled ? '정산묶임' : tx.is_prepaid ? `사전입금${tx.paid_at ? ` (${tx.paid_at.slice(0, 10)})` : ''}` : tx.is_settlement_target ? '정산대상' : '미처리'}
            </Row>
            <Row label="분류 근거">
              <p>{basis}</p>
              <p className="mt-1 text-xs text-gray-500">
                신뢰도 {conf != null ? `${(conf * 100).toFixed(0)}%` : '-'}
                {conf === 1 && ' — 사업자번호 완전 일치(규칙 기반)라 100%로 기록됩니다'}
              </p>
            </Row>
            <Row label="등록/수정">
              <span className="text-xs text-gray-500">{tx.created_at?.slice(0, 19).replace('T', ' ')} / {tx.updated_at?.slice(0, 19).replace('T', ' ')}</span>
            </Row>
            <Row label="거래 ID"><span className="font-mono text-[11px] text-gray-400">{tx.id}</span></Row>
          </dl>
        </div>
      </aside>
    </div>
...
export function TransactionsPage() {
  ...
  const [importOpen, setImportOpen] = useState(() => localStorage.getItem(IMPORT_PANEL_KEY) !== '1')
  ...
  const [viewFilter, setViewFilter] = useState<'unprocessed' | 'target' | 'settled' | 'prepaid' | 'all'>('unprocessed')
  const [selected, setSelected] = useState<Set<string>>(new Set())

  // 목록 조회에 쓰는 필터 — 엑셀 다운로드에도 그대로 넘긴다 (page/limit만 제외)
  const listFilters = {
  ...
  // 'view'는 항상 값이 있으므로 기본값(unprocessed)일 때는 필터 개수에서 제외
  const filterCount = (Object.entries(listFilters) as [string, unknown][])
    .filter(([k, v]) => v !== undefined && v !== '' && v !== false && !(k === 'view' && v === 'unprocessed'))
    .length

  const downloadXlsx = async () => {
    try {
      const resp = await transactionsApi.exportXlsx(listFilters)
      const url = URL.createObjectURL(resp.data as Blob)
      const a = document.createElement('a')
      a.href = url
      const suffix = filterCount > 0 ? '-filtered' : ''
      a.download = `transactions${suffix}-${new Date().toISOString().slice(0, 19).replace(/[:T]/g, '')}.xlsx`
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => transactionsApi.list({ page, limit: 20, ...listFilters }),
  })
  ...
  const { data: deptCountsData } = useQuery({
    queryKey: ['transactions-dept-counts', viewFilter],
    queryFn: () => transactionsApi.departmentCounts(viewFilter),
  })
  ...
  const setTargetMutation = useMutation({
    mutationFn: (vars: { tx_ids: string[]; is_settlement_target: boolean }) => transactionsApi.bulkSetSettlementTarget(vars),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['transactions'] })
      qc.invalidateQueries({ queryKey: ['transactions-dept-counts'] })
      qc.invalidateQueries({ queryKey: ['unsettled-by-vendor'] })
    ...
    onError: (err: any) => alert(err?.response?.data?.detail ?? '정산대상 상태 변경 실패'),
  })
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            title={filterCount > 0
              ? '현재 뷰·부서·검색 조건이 적용된 거래만 내려받습니다'
              : '현재 뷰(미처리)의 거래를 내려받습니다'}
      ...
      <div className="rounded-2xl border border-blue-100 bg-blue-50/50 p-5 shadow-sm">
        <div className="flex items-start justify-between gap-4">
          ...
          {/* 안내가 목록 상단을 계속 차지하지 않도록 접이식 — 접은 상태는 브라우저에 기억 */}
          <button
            type="button"
            onClick={() => {
              const next = !importOpen
              setImportOpen(next)
              localStorage.setItem(IMPORT_PANEL_KEY, next ? '0' : '1')
            }}
            aria-expanded={importOpen}
            className="shrink-0 rounded-lg border border-blue-200 bg-white px-3 py-1 text-xs font-medium text-blue-700 hover:bg-blue-100"
          >
            {importOpen ? '▲ 접기' : '▼ 업로드 열기'}
          </button>
        </div>
        ...
        {importResult && (
          <div className="mt-3 rounded-lg border border-emerald-200 bg-white px-4 py-3 text-xs">
            ...
            {importResult.vendors_created > 0 && (
              <p className="mt-1 text-gray-500">
                신규 거래처 {importResult.vendors_created}개 등록
                {importResult.vendors_auto_dept_assigned > 0 &&
                  ` · 이메일 매칭으로 ${importResult.vendors_auto_dept_assigned}개에 담당부서 자동 지정`}
              </p>
            )}
            <p className="mt-1 text-gray-500">
      ...
      <div className="flex flex-wrap gap-3">
        ...
        <select value={viewFilter} onChange={e => { setViewFilter(e.target.value as any); setPage(1) }}
          ...
          <option value="target">정산대상 (정산관리 포함)</option>
        ...
        vendors={
          deptTab === DEPT_ALL_KEY
            ...
            : deptTab === DEPT_UNASSIGNED_KEY
              ? vendors.filter(v => !v.department_id)
              : vendors.filter(v => v.department_id === deptTab)
        }
        applied={search}
        ...
        <div className="flex flex-wrap gap-1 -mb-px">
          {(() => {
            ...
            return deptTabs.map(t => {
              ...
              // 0건 탭은 흐리게 — 시각적 노이즈 감소 (클릭은 그대로 가능)
              const isEmpty = t.count === 0 && !active
              return (
                <button
                  key={t.key}
                  onClick={() => { setDeptTab(t.key); setPage(1); setSearch(prev => prev.vendor_id ? { ...prev, vendor_id: undefined } : prev) }}
                  title={isEmpty ? '현재 뷰에 해당 부서 거래가 없습니다' : undefined}
                  className={clsx(
                    'flex items-center gap-2 border-b-2 px-4 py-2.5 text-sm font-medium transition-colors',
                    active
                      ? palette.tabActive
                      : 'border-transparent text-gray-500 hover:text-gray-800',
                    isEmpty && 'opacity-40 hover:opacity-100',
                  )}
                >
      ...
      {selected.size > 0 && (
        <div className="flex flex-wrap items-center gap-3 rounded-2xl border border-amber-200 bg-amber-50/60 px-5 py-3 shadow-sm">
          ...
          <button
            onClick={() => {
              if (confirm(`선택한 ${selected.size}건을 정산대상으로 표시합니다.\n미처리 목록에서 빠지고 '정산대상' 탭으로 이동합니다.`)) {
                setTargetMutation.mutate({ tx_ids: Array.from(selected), is_settlement_target: true })
              }
            }}
          ...
          <button
            onClick={() => {
              if (confirm(`선택한 ${selected.size}건을 미처리로 되돌립니다.\n('정산대상' 표시 해제)`)) {
                setTargetMutation.mutate({ tx_ids: Array.from(selected), is_settlement_target: false })
              }
            }}
            disabled={setTargetMutation.isPending}
          ...
          <tbody className="divide-y divide-gray-50">
            {isLoading
              ...
              : transactions.map(tx => {
                  ...
                  return (
                    <tr key={tx.id} className={clsx('hover:bg-gray-50 transition-colors', isChecked && 'bg-blue-50/30', tx.is_prepaid && 'bg-amber-50/30', !tx.is_prepaid && tx.is_settlement_target && 'bg-blue-50/20')}>
                      ...
                      <td className="px-4 py-3.5 max-w-xs">
                        <button
                          ...
                          title="클릭하여 상세 보기"
                          className="block w-full truncate text-left text-gray-900 hover:text-blue-700 hover:underline"
                        >
                          {tx.description ?? '-'}
                        </button>
                            ...
                            <button
                              ...
                              className={clsx(
                                ...
                                tx.is_prepaid
                                  ...
                                  : tx.is_settlement_target
                                    ? 'bg-blue-100 text-blue-700 hover:bg-amber-100 hover:text-amber-800'
                                    : 'bg-gray-100 text-gray-500 hover:bg-amber-100 hover:text-amber-800',
                              )}
                              title="클릭하여 사전입금 전환 (정산대상 표시는 상단 버튼 사용)"
                            >
                              {tx.is_prepaid ? '✓ 사전입금' : tx.is_settlement_target ? '정산대상' : '미처리'}
                            </button>
                          )}
                      ...
                      <td className="px-4 py-3.5">
                        <button
                          type="button"
                          onClick={() => setDetailTx(tx)}
                          aria-label={`${tx.transaction_date} 거래 상세 보기`}
                          className="rounded-lg border border-gray-200 px-2 py-1 text-xs text-gray-600 hover:border-blue-300 hover:bg-blue-50 hover:text-blue-700"
                        >
                          상세
                        </button>
                      </td>
            ...
            {!isLoading && transactions.length === 0 && (
              <tr>
                <td colSpan={14} className="px-5 py-12 text-center">
                  ...
                  <p className="mt-2 text-sm text-gray-500">조건에 맞는 거래가 없습니다.</p>
                  <p className="mt-0.5 text-xs text-gray-400">뷰·부서 탭이나 검색 조건을 바꿔보세요.</p>
                </td>
              </tr>
            )}
          </tbody>
      ...
      {detailTx && (
        <TransactionDrawer
          tx={detailTx}
          vendor={detailTx.vendor_id ? vendorMap[detailTx.vendor_id] : null}
          dept={detailTx.department_id ? deptMap[detailTx.department_id] : null}
