...
export const invoicesApi = {
  ...
  bulkSend: (invoice_ids: string[]) =>
    apiClient.post<{
      success: boolean
      data: { updated: number; skipped_no_account?: number; skipped_vendors?: string[] }
    }>('/api/v1/invoices/bulk-send', { invoice_ids }),
  ...
  updatePaidDate: (id: string, paid_at: string) =>
    apiClient.patch<{ success: boolean; data: Invoice }>(`/api/v1/invoices/${id}/paid-date`, { paid_at }),
