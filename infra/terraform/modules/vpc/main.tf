...
resource "aws_eip" "nat" {
  count  = length(var.availability_zones)
  domain = "vpc"
  tags   = { Name = "${var.app_name}-${var.environment}-nat-eip-${count.index + 1}" }
}
...
resource "aws_nat_gateway" "main" {
  ...
  tags = { Name = "${var.app_name}-${var.environment}-nat-${count.index + 1}" }
}
...
resource "aws_route_table" "private" {
  ...
  tags = { Name = "${var.app_name}-${var.environment}-private-rt-${count.index + 1}" }
}
