...
from app.schemas.settlement import (
    ...
    SettlementCreate,
    SettlementCreateFromTransactions,
    SettlementOut,
    SettlementReview,
)
...
@router.get("/unsettled-transactions", response_model=ApiResponse[list[dict]])
async def unsettled_transactions(
    vendor_id: str = Query(..., description="공급자 ID"),
    db: AsyncSession = Depends(get_db),
    user: User = Depends(current_user),
):
    """특정 공급자의 미정산 거래 상세 목록 — 사용자가 거래 단위로 선택해 정산하기 위한 데이터."""
    service = SettlementService(db)
    scoped_dept = user.department_id if user.role != UserRole.admin else None
    txs = await service.unsettled_transactions(vendor_id=vendor_id, department_id=scoped_dept)
    return ApiResponse(
        success=True,
        data=[
            {
...
@router.post(
    ...
    response_model=ApiResponse[SettlementOut],
    status_code=201,
)
async def create_settlement_from_transactions(
    body: SettlementCreateFromTransactions,
    db: AsyncSession = Depends(get_db),
    user: User = Depends(current_user),
):
    """사용자가 선택한 거래 ID들로 한 건의 정산을 만든다."""
    service = SettlementService(db)
    scoped_dept = user.department_id if user.role != UserRole.admin else None
    s = await service.create_from_transactions(
        vendor_id=body.vendor_id,
        transaction_ids=body.transaction_ids,
        notes=body.notes,
        scoped_department_id=scoped_dept,
    )
    return ApiResponse(success=True, data=SettlementOut.model_validate(s))
