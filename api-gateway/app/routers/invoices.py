...
async def bulk_send_invoices(
    ...
    if not allowed:
        return ApiResponse(success=True, data={"updated": 0, "skipped_no_account": 0, "skipped_vendors": []})

    # 계좌 미등록 공급자의 청구서는 발송 대상에서 제외 (결제 단계 되돌림 방지)
    v_r = await db.execute(select(Vendor).where(Vendor.id.in_({i.vendor_id for i in allowed})))
    vendor_map = {v.id: v for v in v_r.scalars().all()}
    sendable, skipped_names = [], []
    for inv in allowed:
        vendor = vendor_map.get(inv.vendor_id)
        if InvoiceService.has_bank_account(vendor):
            sendable.append(inv)
        else:
            skipped_names.append(vendor.name if vendor else inv.invoice_no)

    skipped_vendors = sorted(set(skipped_names))
    if not sendable:
        return ApiResponse(success=True, data={
            "updated": 0, "skipped_no_account": len(skipped_names), "skipped_vendors": skipped_vendors,
        })

    now_naive = datetime.now(timezone.utc).replace(tzinfo=None)
    await db.execute(
        update(Invoice).where(Invoice.id.in_([i.id for i in sendable]))
        .values(status=InvoiceStatus.sent, sent_at=now_naive)
    )
    await db.commit()
    return ApiResponse(success=True, data={
        "updated": len(sendable),
        "skipped_no_account": len(skipped_names),
...
class PaidDateBody(BaseModel):
    paid_at: date_cls


@router.patch("/{invoice_id}/paid-date", response_model=ApiResponse[InvoiceOut])
async def update_paid_date(
    ...
    """결제완료된 청구서의 결제완료일(paid_at)을 수정한다. admin 전용.

    status는 paid로 유지/전환되며, 묶인 정산도 paid로 자동 전환된다.
    """
    ...
    if inv.status == InvoiceStatus.cancelled:
        raise HTTPException(status_code=409, detail="취소된 청구서는 결제완료일을 수정할 수 없습니다.")

    inv.paid_at = datetime.combine(body.paid_at, datetime.min.time())
