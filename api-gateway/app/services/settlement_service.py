...
class SettlementService:
    ...
    async def _create_one(
        ...
        existing_r = await self._db.execute(
    ...
    async def unsettled_transactions(
        ...
        """특정 공급자의 미정산 거래 상세 목록 (사용자가 거래 단위로 선택해 정산할 수 있도록)."""
        existing_r = await self._db.execute(
            select(Settlement.matched_transactions).where(Settlement.status != SettlementStatus.rejected)
        )
        already_matched: set[str] = set()
        ...
        q = q.order_by(Transaction.transaction_date.asc())

        r = await self._db.execute(q)
        return [t for t in r.scalars().all() if str(t.id) not in already_matched]

    async def create_from_transactions(
        self,
        vendor_id: str,
        ...
        """사용자가 선택한 거래만으로 정산을 생성한다.

        검증:
            - 모든 transaction이 같은 vendor_id에 속해야 함
            - 모든 transaction이 미정산 + 사전입금 아님 + classified/approved 상태
            - scoped_department_id가 주어지면 모든 transaction이 해당 부서여야 함 (member 본인 부서 한정)
            - 이미 다른 정산에 묶인 거래가 있으면 409
        """
        if not transaction_ids:
            raise ConflictError("최소 1건 이상의 거래를 선택해야 합니다.")
        ...
        dup = [tid for tid in ids if tid in already_matched]
        if dup:
            raise ConflictError(f"이미 다른 정산에 포함된 거래가 있습니다 ({len(dup)}건).")

        tx_r = await self._db.execute(select(Transaction).where(Transaction.id.in_(ids)))
        txs = list(tx_r.scalars().all())
        found_ids = {str(t.id) for t in txs}
        missing = [tid for tid in ids if tid not in found_ids]
        if missing:
            raise NotFoundError("Transaction", missing[0])

        for t in txs:
            if t.vendor_id != vendor_id:
                raise ConflictError("선택한 거래 중 공급자가 다른 거래가 있습니다.")
            if t.is_prepaid:
                raise ConflictError("사전입금 거래는 정산 대상이 될 수 없습니다.")
            if t.status not in (TransactionStatus.approved, TransactionStatus.classified):
                raise ConflictError("승인되지 않은 거래가 포함되어 있습니다.")
            if scoped_department_id and t.department_id != scoped_department_id:
                raise ConflictError("본인 부서가 아닌 거래가 포함되어 있습니다.")

        total = float(sum(t.amount for t in txs))
        tx_dates = [t.transaction_date for t in txs]
        settlement = Settlement(
            # 마이크로초 + uuid hex로 같은 초 연속 호출(각각 개별 생성) 시 UNIQUE 충돌 방지
            settlement_no=f"STL-{datetime.now(timezone.utc).strftime('%Y%m%d%H%M%S%f')}-{uuid.uuid4().hex[:4].upper()}",
            vendor_id=vendor_id,
            department_id=vendor.department_id,
            period_start=min(tx_dates),
            period_end=max(tx_dates),
            total_amount=total,
            matched_transactions=ids,
            notes=notes,
        )
        self._db.add(settlement)
        await self._db.commit()
