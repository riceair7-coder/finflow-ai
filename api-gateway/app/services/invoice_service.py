...
class InvoiceService:
    ...
    async def _build_invoice_no(self, department_id: str | None) -> str:
        # 형식: INV-YYMMDD-{DEPT_CODE}-{HEX4}
        # 부서 미지정 시 부서 세그먼트 생략: INV-YYMMDD-{HEX4}
        dept_code: str | None = None
        if department_id:
            row = await self._db.execute(
                select(Department.code).where(Department.id == department_id)
            )
            dept_code = row.scalar_one_or_none()
        date_part = datetime.now(timezone.utc).strftime("%y%m%d")
        suffix = secrets.token_hex(2).upper()
        if dept_code:
            return f"INV-{date_part}-{dept_code}-{suffix}"
    ...
    async def create(self, data: InvoiceCreate) -> Invoice:
        ...
        # 공급자 부서를 invoice_no의 부서 코드로 사용
        v_row = await self._db.execute(
            select(Vendor.department_id).where(Vendor.id == data.vendor_id)
        )
        vendor_dept_id = v_row.scalar_one_or_none()
        invoice_no = await self._build_invoice_no(vendor_dept_id)
    ...
    @staticmethod
    def has_bank_account(vendor: Vendor | None) -> bool:
        """공급자에 입금 계좌가 등록되어 있는지."""
        info = (vendor.bank_info if vendor else None) or {}
        return bool(str(info.get("account_no") or "").strip())
    ...
    async def send(self, invoice_id: uuid.UUID) -> Invoice:
        ...
        # 계좌 미등록 상태로 발송하면 결제 단계에서 반드시 되돌아온다 → 사전 차단
        v_r = await self._db.execute(select(Vendor).where(Vendor.id == invoice.vendor_id))
        vendor = v_r.scalar_one_or_none()
        if not self.has_bank_account(vendor):
            raise ConflictError(
                f"'{vendor.name if vendor else '해당 공급자'}'의 입금 계좌가 등록되지 않아 발송할 수 없습니다. "
                "공급자 관리에서 계좌를 먼저 등록하세요."
            )
    ...
    async def create_from_settlements(
        ...
        invoice_no = await self._build_invoice_no(department_id)
        first = settlements[0]
        invoice = Invoice(
            invoice_no=invoice_no,
            vendor_id=vendor_id,
